title = 'Simple color rules with custom channels example'
description = 'This example shows how to apply color to a <code>CLabel</code> and a <code>CLed</code> based on the ' \
              'value from different channels than the one displayed by the widget. Device named "DemoDevice" has a ' \
              'property "Acquisition" to retrieve data, "Color" ' \
