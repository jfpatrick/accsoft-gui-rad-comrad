...
description = 'This example shows how to create a display with Qt Designer, while still modifying certain parts in ' \
              'code. For instance, in this example the UI is laid out in Qt Desinger, while JAPC connection is ' \
              'specified in code. Device named "DemoDevice" has a single property "Acquisition" with a single string ' \
              'field "Demo", which toggles Tick-Tock labels once a second. In order to access widgets as instance ' \
