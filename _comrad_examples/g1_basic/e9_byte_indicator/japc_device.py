...
class DemoDevice(Device):
    ...
    MyEnum = IntEnum('MyEnum', names=[f'OPTION{idx}' for idx in range(2 ** num_bits)], start=0)  # type: ignore  # https://github.com/python/mypy/issues/8219
    MyBitMask = IntFlag('MyBitMask', names=[f'BIT{idx}' for idx in range(num_bits)])  # type: ignore  # https://github.com/python/mypy/issues/8219
